...
public class ReportPanel extends JPanel {
	...
	private Map<Organisation, Map<Boolean, Integer>> organisationResults;
	...
	private void initOrgTbl() {
		orgTbl = new JTable(new DefaultTableModel() {
			private static final long serialVersionUID = 1L;
			private String[] columns = { "Organisation Name", "Open Vacancies", "Closed Vacancies" };

			@Override
			public Object getValueAt(int row, int col) {
				Organisation organisation;

				if (organisationResults.size() != 0) {
					organisation = organisations.get(row);
					switch (col) {
					case 0:
						return organisation.getOrganisationName();
					case 1:
						return organisationResults.get(organisation).get(true);
					case 2:
						return organisationResults.get(organisation).get(false);
					}
				}
				return null;
			}
	...
	public void updateOrganisationReport(Map<Organisation, Map<Boolean, Integer>> results) {
		this.organisationResults = results;
		this.organisations = new ArrayList<>(results.keySet());
