...
/**
 * Dialog that allows the user to edit a user.
 * @author Charlie
 */
public class EditUserDialog extends RecruitmentDialog {
	...
	public User getUser() {
		...
		if(errorMessage.toString().trim().equals("")) {
			...
			JOptionPane.showMessageDialog(this, errorMessage.toString(), "Cannot edit user", JOptionPane.ERROR_MESSAGE);
			return null;
		}
	}
	...
	public void setDisplayedUser(User user) {
		userIdTxtField.setText(user.getUserId());
		firstNameTxtField.setText(user.getFirstName());
		surnameTxtField.setText(user.getSurname());
		emailTxtField.setText(user.getEmailAddress());
		phoneNoTxtField.setText(user.getPhoneNumber());
		if (user.getAccountStatus()) {
			statusCmbBox.setSelectedIndex(0);
		} else {
			statusCmbBox.setSelectedIndex(1);
		}
		accountTypeCmbBox.setSelectedItem(user.getAccountType());
		passwordTxtField.setText("       ");
		confirmPasswordTxtField.setText("       ");
	}
